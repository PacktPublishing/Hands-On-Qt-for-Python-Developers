...
class UApp(UWindow, UTools):                   # Create the main class.
    ...
    def data(self, action):                    # Function for the "Data"
        ...
        if self.actx == "CSV":                 # as text. The "Shelve" loads
            try:                               # data as dictionary values.
                ...
                self.stat_bar.showMessage(self.actx + ' ' + str(err))
        if self.actx == "Pandas":              # values from the CSV file
            try:                               # with python standard lib,
                pread = self.pandas_read(filename="data/ptests.csv",
                                         reader="csv", sep=',')    
                self.text_edit.append(str(pread[0]))
                self.stat_bar.showMessage(self.actx + " opened")
            except Exception as err:           # "Pandas" reads the CSV,
                self.stat_bar.showMessage(self.actx + ' ' + str(err))
    ...
    def user_data4(self, uname, umail, upass, udata):
        try:                                   # Writing CSV with pandas.
            ...
            uindex = [i for i in range(1000)]  # Creates file with 4 columns.
            udata = {"User_Name": range(0, 1000), "User_email": range(0, 1000),
                     "User_password": range(0, 1000), "User_data": range(0, 1000)}
            self.pandas_write(filename="data/ptests.csv", writer="csv",
